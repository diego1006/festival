...
function cargar(){
    ...
    datos.append("listarRechazados", true);
        ...
        success: function (respuesta) {
            respuesta.forEach(function(element, index) {
                ...
                tabla.row.add([
                    ...
                    '<div class="form-group"><button class="btn btn-sm btn-secondary" style="color:white;border-radius:20px;width:90%" data-toggle="modal" title="Eliminar" data-target="#modalVer" onclick="mostrarById(' + element.id + ')">Rechazado</button></div>'
                ]).draw(false);
            })
        }
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            if(respuesta == "ok"){
                ...
                datos2.append("correo", $("#correo").html());
                $.ajax({
